...
export function MobileProjectWall({
  ...
  const didInitialSettleRef = useRef(false)   // PART2: 진입/필터 후 랜덤 중앙 정착 1회 게이트
  ...
  useEffect(() => {
    ...
    if (firstRevealRef.current) {
      ...
      // unfolding 과도기 생략하고 즉시 idle 직행 — 중앙 랜덤 정착은 PART2(settleInitialRandom)가 담당
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      setPhase('pre')
      didInitialSettleRef.current = false   // PART2-2(b): 다음 idle 도달 시 settleInitialRandom 재실행 (랜덤 중앙 정착)
  ...
  const shuffleFlagTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)   // PART3: scrollIntoView 후 programmaticRef 해제 타이머
  ...
  // PART2 — 진입/필터 직후: 셔플 큐의 첫 항목(랜덤)을 화면 수직 중앙으로 즉시 정착 (애니메이션 없이)
  const settleInitialRandom = useCallback(() => {
    ...
    // 0번을 "이미 보여준 것"으로 간주하고 1부터 다음 셔플 진행
    queueIdxRef.current = 1
    lastShuffleRef.current = Date.now()   // 진입 직후 즉시 셔플 방지 (6초 간격 확보)
    updateCenter()                        // PART2-3: centerIdx를 랜덤 첫 항목(d=0)으로 동기화
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [displayList])

  // PART2-2(a): 최초 진입/필터 후 idle 도달 시 랜덤 중앙 정착 (큐·itemEls 준비 후 rAF 2회)
  useEffect(() => {
  ...
  const advanceShuffle = useCallback(() => {
    ...
    // PART3: 셔플 스크롤을 데스크톱과 동일하게 scrollIntoView(smooth, center)로 — 거리 가변·네이티브 속도
    // scrollIntoView는 programmaticRef를 세우지 않으므로 수동으로 set, 종료 추정 시간(700ms) 후 해제
    programmaticRef.current = true
  ...
  useEffect(() => () => {
    ...
    if (closeTimerRef.current) clearTimeout(closeTimerRef.current)
    if (shuffleFlagTimerRef.current) clearTimeout(shuffleFlagTimerRef.current)   // PART3
  }, [cancelScrollAnim])
